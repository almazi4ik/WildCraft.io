...
socket.on('connect', () => {
    document.getElementById('connection-status').innerText = "Подключено! Можно играть.";
    document.getElementById('connection-status').style.color = "#5cb85c";
});
socket.on('disconnect', () => {
    document.getElementById('connection-status').innerText = "Потеряно соединение с сервером";
    document.getElementById('connection-status').style.color = "#ff4444";
});

function startGame() {
    if (!socket.connected) {
        alert("Подожди подключения к серверу Render!");
        return;
    }
...
function gameLoop() {
    if (!gameActive) return;
    socket.emit('player_input', { move: { x: (keys.a ? -1 : 0) + (keys.d ? 1 : 0), y: (keys.w ? -1 : 0) + (keys.s ? 1 : 0) }, angle: mouseAngle });
...
function drawGrid(c, cx, cy) {
    c.strokeStyle = '#4d7a2e'; c.lineWidth = 2; const size = 100;
    for (let x = Math.floor(cx / size) * size; x < cx + canvas.width + size; x += size) { c.beginPath(); c.moveTo(x - cx, 0); c.lineTo(x - cx, canvas.height); c.stroke(); }
    for (let y = Math.floor(cy / size) * size; y < cy + canvas.height + size; y += size) { c.beginPath(); c.moveTo(0, y - cy); c.lineTo(canvas.width, y - cy); c.stroke(); }
}
function drawObject(c, obj, cx, cy) {
    const sx = obj.x - cx, sy = obj.y - cy;
    if (sx < -100 || sx > canvas.width + 100 || sy < -100 || sy > canvas.height + 100) return;
    c.beginPath(); c.arc(sx, sy, obj.radius, 0, Math.PI * 2);
    c.fillStyle = obj.type === 'tree' ? '#2e5c1e' : '#7a7a7a'; c.strokeStyle = obj.type === 'tree' ? '#1f3d14' : '#555';
...
function drawRadar(me) {
    ...
    for (let id in players) {
        if (id !== myId) {
            ...
            radarCtx.beginPath(); radarCtx.arc((players[id].x / mapSize) * 150, (players[id].y / mapSize) * 150, 3, 0, Math.PI * 2); radarCtx.fill();
        }
    }
    radarCtx.fillStyle = '#ffffff'; radarCtx.beginPath(); radarCtx.arc((me.x / mapSize) * 150, (me.y / mapSize) * 150, 4, 0, Math.PI * 2); radarCtx.fill();
}
